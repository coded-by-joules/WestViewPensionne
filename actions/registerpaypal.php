...
//end of encrytion
if ($Password==$RePass) {
	$query_verify_email = "SELECT * FROM tblusers WHERE emailaddress = '$Email'";
	$verified_email = $conn->query($query_verify_email);
	if ($verified_email->num_rows == 0) {
		$hash = md5(uniqid(rand(), true));
		$sql = "INSERT INTO tblusers (name,lastname,username,password,phonenumber,emailaddress,gender,city,country,type,active,verification,hash)
		...
		if($conn ->query($sql) === TRUE) {
            ...
            if ($conn->affected_rows==1) {
                ...
                $url= 'localhost/OnlineReservationv.02.3/verify.php?emailaddress=' . urlencode($Email) . '&key='.$hash.'';
