...
function validAction(){
        ...
        var dataString = 'reservationid=' + reservationid + '&customerid=' + customerid  + '&confirmation='+ confirmation 
                        + '&status=' + status + '&downpayment=' + downpayment + '&addonpayable=' + addonpayable + '&payable=' + payable + '&type=' + type ;
        $.ajax({
        type: "POST",
...
if(searchstartdate!='' || searchstartdate=='' && searchenddate !='' || searchenddate=='')
{
  $.ajax({
  type: "POST",
  url: "../CustomerReservation/searchroomdetail.php",
  data: dataString,
  cache: false, 
  success: function(html)
  {
...
if(searchreservationid!='' || searchreservationid=='')
{
  ...
  function validTransfer(){
        ...
        var categoryname=$('#roomtype').text();
        var rate=$('#amount').text();
        var arrival=$('#sd').text();
        var departure=$('#ed').text();
        var type=$('#type').text();
     
        var dataString ='roomid='+ roomid + '&categoryname=' + categoryname + '&rate=' + rate + '&arrival=' + arrival + '&departure=' + departure + '&type=' + type ;
        $.ajax({
        type: "POST",
        url: "../CustomerReservation/Transfer.php",
        data: dataString,
...
function reserveDelete(id) {
  $("#delreserve").click(function(e){
    window.location='deletereserve.php?resid='+ id;
  });
}
